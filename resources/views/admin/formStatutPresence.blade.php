...
<body>
    ...
    <div class="form-wrapper">
        <form action="{{ route('admin.statutpresence.store') }}" method="post" novalidate>
            @csrf

            <a href="{{ route('admin.statutpresence.liste') }}" class="statutPrésence-list-link" title="Listes des statut présences">
                listes des Statut présence
            ...
            <input type="text" name="nom_statut_presence" placeholder="Nom du statut de présence" required value="{{ old('nom_seance') }}" />

            @error('nom_statut_presence')
                <p style="color: #f44336; font-size: 12px;">{{ $message }}</p>
            @enderror

            <button type="submit">Ajouter un statut présence</button>
